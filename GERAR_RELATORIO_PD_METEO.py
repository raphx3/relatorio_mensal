...
def exibir_pagina_streamlit():
    ...
    # Obter o diretório de trabalho atual usando os.getcwd()
    ...
    if st.button(label='Gerar Resultados para Relatório na Pasta Selecionada'):
        if pasta_saida:
            # Combina o caminho fornecido com o diretório atual, caso seja necessário
            caminho_completo = os.path.join(cwd, pasta_saida)
            
            # Verifica se o diretório existe, se não, cria o diretório
            try:
                os.makedirs(caminho_completo, exist_ok=True)  # Cria o diretório se não existir
                st.success(f"Pasta criada em: {caminho_completo}")
            ...
            try:
                ...
                # Supondo que você tenha os DataFrames de dados e as variáveis de parâmetros
                df = pd.DataFrame({
                ...
                plot_series_temporais(df_filtrado_por_tempo, None, parametro_para_teste, os.path.join(caminho_completo, parametro_para_teste))
                ...
                output_file = os.path.join(caminho_completo, f'relatorio_{parametro_para_teste}.csv')
